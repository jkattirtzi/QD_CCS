...
!
      ...
      INTEGER :: fu
      INTEGER SS,MM,HH,DD,FILNO,I
      INTEGER INIT
      DOUBLE PRECISION ZBQLIX(43),B,C
      DOUBLE PRECISION TMPVAR1,DSS,DMM,DHH,DDD
      
      INTEGER :: myfu ! file unit to write messages to
      COMMON /ZBQL0001/ ZBQLIX,B,C
      SAVE INIT
...
!       write(myfu,*) "sysdate", SS,MM,HH,DD
       ...
       call idate(today)        ! today(2)=day, (1)=month, (3)=year
...
!         if(present(fu)) then
         if(fu .ge. 0) then
!            Print *, 'oi Im here right now'  ! written by JK
            write(fu,*) "  Seed ", SEED   ! I've (JK) changed this from myfu to fu             
         endif
